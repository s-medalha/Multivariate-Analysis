...
In order to find the latent variables behind the answers to the survey, we will perform factor analysis on our data set. Before performing dimensional reduction, we considered the correlation matrix of our data:

```{r}
#| label: corr-matrix
cor(scaled_data)
cor(scaled_data) |> round(2) |> image()
```

We can see that each variable has high correlation with at least one other variable. To evaluate the suitability of factor analysis, we can perform the Kaiser-Meyer-Olkin test. 
...
After executing the test, the MSA (measure of sampling accuracy, which varies between 0 and 1) is calculated as `r format(KMO(scaled_data)$MSA, digits = 2)`. This is classified as "meritorious", so we have an indication that factor analysis is suitable for this data set.

In order to decide the number of factors, we started by performing factor analysis with a high number of values and determined the scree plot.
...
```
Since there are three eigenvalues above 1 (Kaiser criterion) and the elbow of the scree plot is at the fourth component, we decided to use 3 factors in our factor analysis.

In order to decide which method (principal components factoring or principal axis factoring) and rotation (varimax or quartimax) to use, we   
